...
```{r extra_models, message=FALSE, warning=FALSE, paged.print=FALSE}
...
``` {r leftover_from_testing_prpchitcds}
# the data are basically bimodal, so a beta regression won't work

# requires that 0 < y < 1: so we transform (y * (n−1) + 0.5) / n where n = sample size
propchitcds.rand$propchitcds_sqz <- (propchitcds.rand$prop_sa.tds * (nrow(propchitcds.rand)-1) + 0.5)/
  nrow(propchitcds.rand)
propchitcds.tt$propchitcds_sqz <- (propchitcds.tt$prop_sa.tds * (nrow(propchitcds.tt)-1) + 0.5)/
  nrow(propchitcds.tt)

## prop chi TCDS random sample ####
prpchitcds.rand.beta <- glmmTMB(propchitcds_sqz ~
                           tchiyr.std +
                           (1|aclew_child_id), # max: no cnvgnce w/ stthr slopes
                         data=propchitcds.rand,
                         family=beta_family(link = "logit"))
#qqnorm(residuals(prpchitcds.rand.beta))
#qqline(residuals(prpchitcds.rand.beta)) # D: !!

## prop chi TCDS tt sample ####
prpchitcds.tt.beta <- glmmTMB(propchitcds_sqz ~
                                tchiyr.std +
                           (1|aclew_child_id), # max: no cnvgnce w/ stthr slopes
                         data=propchitcds.tt,
                         family=beta_family(link = "logit"))

#qqnorm(residuals(prpchitcds.tt.beta))
#qqline(residuals(prpchitcds.tt.beta)) # D: !!

```


xds.speakertype <- tibble(
  spkrtype = c("CHI", "A", "C", "U", "E"),
  xdsvals = c("T", "[AB]", "[CB]", "[U]", "[O]")
)
    ...
    if (nrow(subdata.chi) > 0) {
      ...
      for (j in 1:nrow(chi.vocs)) {
        ...
        while (stop.looking.left == FALSE) {
          ...
          if (curr.spk == "CHI") {
            candidate.vocs <- subdata.tcds %>%
              filter(stop >= curr.start - allowed.gap &
                     stop <= min(curr.stop, (curr.start + allowed.overlap))) %>%
              arrange(start)
          } else {
            candidate.vocs <- subdata.chi %>%
              filter(stop >= curr.start - allowed.gap &
                       stop <= min(curr.stop, (curr.start + allowed.overlap))) %>%
              arrange(start)
          }
          ...
          } else { # otherwise look for speaker continuations;
            # speech from the current speaker occurring before the current turn,
            # that ends within the max gap window preceding the current turn start
            candidate.continuation.vocs <- subdata %>%
              filter(speaker == curr.spk &
                       stop <= curr.start &
        ...
        while(stop.looking.right == FALSE) {
          if (curr.spk == "CHI") {
            candidate.vocs <- subdata.tcds %>%
              filter(start >= max(curr.start, (curr.stop - allowed.overlap)) &
          ...
          candidate.continuation.vocs <- tibble()
          if (nrow(candidate.vocs) > 0) {
            # continue with the latest (last occurring) candidate
            curr.start <- candidate.vocs$start[1]
            curr.stop <- candidate.vocs$stop[1]
            curr.spk <- candidate.vocs$speaker[1]
          } else { # otherwise look for speaker continuations;
            # speech from the current speaker occurring after the current turn,
            # that starts within the max gap window following the current turn end
            candidate.continuation.vocs <- subdata %>%
              filter(speaker == curr.spk &
                       start >= curr.stop &
                       start <= curr.stop + allowed.gap)
