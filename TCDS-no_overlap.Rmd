...
# for calculating transitions and waking time
...
for (i in 1:length(ts.files)) {
  newfile <- read_csv(paste0(ts.data.path, files[i]),
...
ts.ptcp.info <- ts.ptcp.info %>%
  mutate(
    ...
    motyr.m = mean(mother_age), # diff col name yd
    nsb.m = mean(number_older_sibs),
    hsz.m = mean(household_size),
    tchiyr.sd = sd(age_mo_round),
    motyr.sd = sd(mother_age), # diff col name yd
...
for (i in 1:length(yd.files)) {
  newfile <- read_csv(paste0(yd.data.path, files[i]),
                      col_types = cols(val = col_character()))
  newfile$aclew_child_id <- unlist(strsplit(files[i], '\\.'))[1]
...
yd.ptcp.info <- yd.ptcp.info %>%
  mutate(
    tchiyr.m = mean(age_mo_round),
    motyr.m = mean(mat_age), # diff col name ts
    nsb.m = mean(number_older_sibs),
    hsz.m = mean(household_size),
    tchiyr.sd = sd(age_mo_round),
    motyr.sd = sd(mat_age), # diff col name ts
    nsb.sd = sd(number_older_sibs),
    hsz.sd = sd(household_size)
    )
...
for (ptcp in ts.unique.ptcp) {
  tcds.utts <- ts.all.data %>%
    filter(aclew_child_id == ptcp &
      sample == "random" & speaker != "CHI" &
        grepl("xds@", tier) & val == "T") %>%
    mutate(nonovlp.dur = NA)
  non.tcds.utts <- ts.all.data %>%
    filter(sample == "random" &
        (grepl("xds@", tier) | tier == "CHI") & # include child vocs and ODS
        val != "T")
  for (i in 1:nrow(tcds.utts)) {
    ovlp.utts <- which(
      non.tcds.utts$stop > tcds.utts$start[i] &
        non.tcds.utts$start < tcds.utts$stop[i])
    if (length(ovlp.utts) > 0) {
      # complete overlap
      if (length(which(
        non.tcds.utts$start[ovlp.utts] <= tcds.utts$start[i] &
        non.tcds.utts$stop[ovlp.utts] >= tcds.utts$stop[i]
        )) > 0) {
          tcds.utts$nonovlp.dur[i] <- 0
      } else { # msec by msec check
        msec.count <- 0
        tcds.msecs <- tcds.utts$start[i]:tcds.utts$stop[i]
        ovlp.msecs <- c()
        for (j in length(ovlp.utts)) {
          ovlp.msecs <- c(ovlp.msecs,
            non.tcds.utts$start[ovlp.utts[j]]:non.tcds.utts$stop[ovlp.utts[j]])
        }
        non.ovlp.msec <- length(which(!(tcds.msecs %in% ovlp.msecs))) - 1
        tcds.utts$nonovlp.dur[i] <- ifelse(non.ovlp.msec > 0, non.ovlp.msec, 0)
      }
    } else {
      tcds.utts$nonovlp.dur[i] <- tcds.utts$dur[i]
    }
  }
  ts.tcds.utts.no.ovlp <- bind_rows(ts.tcds.utts.no.ovlp, tcds.utts)
}
...
for (ptcp in yd.unique.ptcp) {
  tcds.utts <- yd.all.data %>%
    filter(aclew_child_id == ptcp &
      sample == "random" & speaker != "CHI" &
        grepl("xds@", tier) & val == "T") %>%
    mutate(nonovlp.dur = NA)
  non.tcds.utts <- yd.all.data %>%
    filter(sample == "random" &
        (grepl("xds@", tier) | tier == "CHI") & # include child vocs and ODS
        val != "T")
  for (i in 1:nrow(tcds.utts)) {
  ...
  yd.tcds.utyd.no.ovlp <- bind_rows(yd.tcds.utyd.no.ovlp, tcds.utts)
}
