...
```{r random_interaction_prep, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
for (i in 1:nrow(all.segments)) {
  ...
  for (j in 1:nrow(chi.vocs)) {
    ...
    # look for turn transitions that start:
    #    
      tm1.min.offset <- curr.start - allowed.gap
      tm1.max.onset <- curr.start
      xds.pattern <- ifelse(grepl("C", curr.spk), "[T23]", "[AB3]")
      # look first for turn transitions
      candidate.vocs <- data %>%
        filter(grepl("xds", tier) &
                 speaker != curr.spk &
                 grepl(xds.pattern, value) &
                 start <= tm1.max.onset & stop > tm1.min.offset)
      if (nrow(candidate.vocs) > 0) {
        ...
        curr.xds <- filter(data, speaker == curr.spk & grepl("xds", tier) &
                           start == curr.start & stop == curr.stop)$value
      }
      candidate.continuation.vocs <- tibble()
      # If no turn transitions, look for previous turns by the same speaker
      if (nrow(candidate.vocs) == 0) {
        candidate.continuation.vocs <- data %>%
          filter(grepl("xds", tier) &
                   speaker == curr.spk &
                   value == curr.xds &
                   start <= tm1.max.onset & stop > tm1.min.offset &
      ...
      if (nrow(candidate.vocs) == 0 & nrow(candidate.continuation.vocs) == 0) {
...
```{r make_random_interaction_plots, message=FALSE, warning=FALSE, paged.print=FALSE}
# Graph the basic speech quantity info
# XDS min/hr
xdsmph.segments.tt <- ggplot(quantity.nonrand.tt,
                          aes(x = age_mo_round, y = xds_mph)) +
