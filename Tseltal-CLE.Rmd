...
tdsprp.segments.rand_and_tt.sa <- ggplot(quantity.sa.rand_and_tt,
...
grid.arrange(odsmph.segments.rand_and_tt, tdsmph.segments.rand_and_tt.zoomedin,
             tdsprp.segments.rand_and_tt, nspkrs.segments.rand_and_tt, nrow=2, ncol=2)
```

```{r corpus_comparison_plots, message=FALSE, warning=FALSE, paged.print=FALSE}
...
#tchiyr.std                  0.6578     0.2656   2.477  0.01325 *  
#nsk.std                    -0.3441     0.1712  -2.010  0.04446 *  
#tchiyr.std:mated.bin6+     -0.6181     0.2786  -2.219  0.02650 *  
#tchiyr.std:nsk.std          0.5775     0.1934   2.985  0.00283 ** 
#tchiyr.std:I(stthr.std^2)  -3.8966     1.8338  -2.125  0.03359 *  
# ZI:
#I(stthr.std^2)         -162.0801    70.5261  -2.298   0.0216 *
#nsk.std                  -1.2825     0.6723  -1.908   0.0564 .
#I(stthr.std^2):hsz.std  107.8256    53.9103   2.000   0.0455 *
#I(stthr.std^2):nsk.std  -80.2218    43.6716  -1.837   0.0662 .
...
#tchiyr.std:I(stthr.std^2) -4.37920    2.46473  -1.777 0.075610 .  
...
#tchiyr.std:I(stthr.std^2)  -4.3325     2.2614  -1.916   0.0554 .  
```
...
# Nspeaker:
#nsk.std                    -0.3441     0.1712  -2.010  0.04446 *  
#ggplot() + geom_point(data = quantity.rand, aes(x = nsk.std, y = round(tds_mph,0))) + geom_smooth(data = quantity.rand, aes(x = nsk.std, y = round(tds_mph,0)), method = "lm")
...
# Maternal education * chi age:
#tchiyr.std:mated.bin6+     -0.6181     0.2786  -2.219  0.02650 *  
#ggplot() + geom_point(data = quantity.rand, aes(y = round(tds_mph,0), x = age_mo_round, color = mated.bin)) + geom_smooth(data = quantity.rand, aes(y = round(tds_mph,0), x = age_mo_round, color = mated.bin), method = "lm")
...
# ZI - time of day * household size (note ZI estimates are prob of DV being 0 ~ flip for interpretation):
#I(stthr.std^2):hsz.std  107.8256    53.9103   2.000   0.0455 *
#ggplot() + geom_smooth(data = quantity.rand, aes(x = start.hr, y = tds_mph.nz, color = as.factor(hsz.std)), method = "lm", formula = y ~ poly(x, 2))
```
...
tds.tt.zinb <- glmmTMB(round(tds_mph,0) ~
                         ...
                         family="nbinom1")
#summary(tds.tt.zinb)
#nsk.std                   -0.167697   0.097651  -1.717  0.08592 .  
#tchiyr.std:hsz.std        -0.322806   0.172244  -1.874  0.06091 .  
#tchiyr.std:I(stthr.std^2) -4.284506   1.446813  -2.961  0.00306 ** 
# No significant ZI effects
...
#tchiyr.std:nsk.std         0.33195    0.19003   1.747   0.0807 . 
```
