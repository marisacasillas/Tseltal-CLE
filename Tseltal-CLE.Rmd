...
Second, most work on CDS quantity uses summary measures that average over the ebb and flow of the day (e.g., average proportion CDS). In reality, verbal behaviors are highly structured during interaction: while some occur at regular intervals<!-- ('periodic', e.g., discourse connectives such as 'alright', 'okay', and 'well')-->, others occur in shorter, more intense bursts separated by long periods of inactivity<!--['bursty', e.g., content words, descriptions; @abney2018bursts]-->. Infants' and adults' vocal behavior is clustered across multiple time scales of daylong recordings [@abney2017time] and noun and verb use is bursty across languages [@blasiIPhuman]. In experimental settings, two-year-olds have been shown to learn novel words better from a massed presentation of object labels versus a distributed one [@schwab2016repetition]. The existence of multi-scale temporal structure in language exposure implies new roles for attention and memory in development; more work is needed to know how CDS is distributed over children's daily experiences [@soderstrom2013when].
...
```{r fig4, echo=FALSE, fig.cap="TCDS rates reported from at-home recordings in various populations, including urban (empty shape) and rural/indigenous (filled shape) samples. Points show the average TCDS rates across age---their size indicates the number of children sampled (range: 1--26). Data sources: @bergelsoncasillas2019what US/Canada; @shneidman2010language US and Yucatec; @vogt2015communicative Dutch, Mozambique urban and rural; @scaffIPlanguage Tsimane.", message=FALSE, warning=FALSE, fig.height=2.5}
...
tcds.random.distribution <- ggplot(quantity.rand,
  ...
  xlab ("TCDS min/hr") +
  basic.theme
ggsave(paste0(shiny.input.path, "TCDS_random.distribution.png"),
...
tcds.tt.distribution <- ggplot(quantity.nonrand.tt,
  ...
  xlab ("TCDS min/hr") +
  basic.theme
ggsave(paste0(shiny.input.path, "TCDS_turntaking.distribution.png"),
...
ods.random.distribution <- ggplot(quantity.rand,
  ...
  xlab ("ODS min/hr") +
  basic.theme
ggsave(paste0(shiny.input.path, "ODS_random.distribution.png"),
...
ods.tt.distribution <- ggplot(quantity.nonrand.tt,
  ...
  xlab ("ODS min/hr") +
  basic.theme
ggsave(paste0(shiny.input.path, "ODS_turntaking.distribution.png"),
...
c_o.tpm.random.distribution <- ggplot(turn.transitions.rand,
  ...
  basic.theme
ggsave(paste0(shiny.input.path, "c_o.tpm_random.distribution.png"),
...
c_o.tpm.tt.distribution <- ggplot(turn.transitions.tt,
  ...
  xlab ("TC-O transitions/min") +
  basic.theme
ggsave(paste0(shiny.input.path, "c_o.tpm_turntaking.distribution.png"),
...
o_c.tpm.random.distribution <- ggplot(turn.transitions.rand,
  ...
  basic.theme
ggsave(paste0(shiny.input.path, "o_c.tpm_random.distribution.png"),
...
o_c.tpm.tt.distribution <- ggplot(turn.transitions.tt,
  ...
  xlab ("O-TC transitions/min") +
  basic.theme
ggsave(paste0(shiny.input.path, "o_c.tpm_turntaking.distribution.png"),
...
The current findings are based on a cross-sectional analysis of 600 annotated recording minutes, divided among only ten children. The data are limited mainly to verbal activity; we cannot analyze gaze and gestural behavior [@brown2014interactional]. We have also used overall vocal maturity rating as an index of language development, but further work should include receptive and productive measures of linguistic skill, using experiment- and questionnaire- based measures, as well as more in-depth analyses of spontaneous speech, all of which builds on past work [@brown2014interactional; @brown2014language; @brown2011cultural; @brown1998conversational; @brown1998childrens].  In short, more and more diverse data are needed to enrich this initial description of Tseltal children's language environments. Importantly, the current analyses are based on a corpus that is still under active development: as new data are added, up-to-date versions of these analyses will be available with the current data and analysis scripts can be found: ADD-URL.
