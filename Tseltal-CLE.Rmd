...
We annotated video clips from 10 of the 55 children's recordings. We chose these 10 recordings to maximize variance in three demographic variables: child age (0--3;0), child sex, and maternal education. The sample is summarized in *Table 1* *[TASK 09: Make table]*. We then selected one hour's worth of non-overlapping clips from each recording in the following order: nine randomly selected 5-minute clips, five 1-minute clips manually selected as the top 'turn-taking' minutes of the recording, five 1-minute clips manually selected as the top 'vocal activity' minutes of the recording, and one, manually selected 5-minute extension of the best 1-minute sample.
<!--IMPORTANT: Say right here why we have all these sample types!!-->

The manually selected clips were chosen by two trained annotators (the first author and a student assistant) who listened to each recording in its entirety at 1--2x speed and took notes on the onset times of candidate turn taking and vocal activity regions. Then the first author reviewed each candidate closely (1x speed, multiple repetitions) to choose the best five one-minute samples for each of the two types of activity. Good turn-taking activity was defined as at closely timed sequences of contingent vocalization between the target child and at least one other person. The 'best' turn-taking clips were chosen because they had the most and most clear turn-switching activity between the target child and the other speaker(s). Good vocal activity clips were defined as clips in which the target child produced the most and most diverse spontaneous (i.e., not imitative) vocalizations. The 'best' vocal activity clips were chosen for representing the most linguistically mature and/or diverse vocalizations made by the child over the day---the idea of this sample is to get an idea of the child's productive speech abilities. All else being equal, candidate clips were prioritized when they contained less background noise or speakers who were not otherwise frequently represented (e.g., CDS from older males). The best turn-taking clips and vocal activity clips often overlapped; clips were first assigned to turn-taking and vocal activity clips were chosen from the remainder of the candidates.
...
We exported each ELAN file into a tab-separated values, which we could then read into ```R.version$version.string``` for analysis. All plots were made using the ggplot2 package, and all analyses run using the lme4 and betareg packages *[TASK 10: Fix references to packages and their citations]*.
