...
\caption{\label{fig:fig2}By-child estimates for minutes per hour of all
overheard speech (upper left), target-child-directed speech (upper
right), proportion target-child-directed speech (lower left), and
proportion of TCDS coming from adults vs.~children (lower right; adults
= purple). Black dots and boxplots indicate variance in each measure
across the high turn-taking Tseltal clips. Bands on the solid linear
trends show 95\% CIs. Dashed linear trends indicate averages from the
randomly sampled clips for comparison.}
\end{figure}

** TO DO NEXT: ** - Add turn taking figs, descriptors, and models
(somewhere?) for both rand and tt - then\ldots{} replicate this with the
turn-taking sample
