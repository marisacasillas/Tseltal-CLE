...
selected 5-minute extension of the best 1-minute sample. We created
these different subsamples of each day to measure properties of (a)
children's \emph{average} language environments (random samples) and (b)
their \emph{most input-dense} language environments (turn-taking
samples). The third sample (high-activity) gave us insight into
children's productive speech abilities.
...
(target-child-directed/other-child-directed/adult-directed/adult-and-child-directed/animal-directed/other-speaker-type-directed).\footnote{Full documentation, including training materials, for the ACLEW Annotation Scheme can be found at *[TASK 10: Add OSF link]*.}

\subsection{Data analysis}\label{methods-analysisinfo}

We reformatted each ELAN file into tab-separated values so that the
annotations could be read into R version 3.5.0 (2018-04-23) for
analysis. All plots were made using the ggplot2 package, and all
analyses run using the lme4 and betareg packages \emph{{[}TASK 11: Fix
references to packages and their citations{]}}.
