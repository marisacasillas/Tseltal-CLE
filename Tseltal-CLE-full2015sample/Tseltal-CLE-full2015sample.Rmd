  ...
  if (nrow(rec.utt.info) > 0) {
    ...
    if (nrow(cds.utts.wi.is.rec) > 0) {
      cds.utts.wi.is.rec <- cds.utts.wi.is.rec %>%
        ...
        left_join(filter(rec.utt.info, tier.name == "OTR"))
    }
    if (nrow(chi.utts.wi.is.rec) > 0) {
      chi.utts.wi.is.rec <- chi.utts.wi.is.rec %>%
        mutate(time.start = start.ms/1000) %>%
        left_join(filter(rec.utt.info, tier.name == "CHI"))
    }
  }
  if (nrow(cds.utts.wi.is.rec) > 0) {
   utts.within.is <- bind_rows(
     ...
     bind_rows(mutate(chi.utts.wi.is.rec, recording = rec) %>%
       select(-start.ms)) %>%
     arrange(recording, clip.onset, time.start)
  }
}
# take care of imperfectly matched cases (ARGH rounding issue, can't deal)
utt.info$time.start.char <- as.character(utt.info$time.start)
unmatched.rows <- which(is.na(utts.within.is$tier.name))
plus.1ms <- utts.within.is %>%
  filter(is.na(tier.name)) %>%
  mutate(time.start = time.start + .001,
         time.start.char = as.character(time.start)) %>%
  select(clip.onset, time.start.char, recording) %>%
  left_join(filter(utt.info, tier.name == "OTR")) %>%
  select(-time.start.char)
utts.within.is <- utts.within.is %>%
  filter(!is.na(tier.name)) %>%
  full_join(plus.1ms) %>%
  mutate(within.is = TRUE)
utt.info <- utt.info %>%
  full_join(utts.within.is)
...
```{r summarize_isOTR, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# Create summaries, adding in zeroes for annotated clips with no speech

# ... at the clip level
clip.summary.is <- utt.info %>%
  filter(within.is == TRUE) %>%
  group_by(recording, tier.name, clip.onset) %>%
  summarize(
    tot.spch.min = sum(dur.sec)/60,
    min.p.hr = tot.spch.min*(60/clip.dur.min)
  ...
  full_join(
    filter(all.clips.CHIOTR.tiers, tier.name == "OTR"), # no filter w/ chi vocs
            by = c("recording", "tier.name",
                   "clip.onset" = "clip.onset.uncorrected")) %>%
  select(-clip.onset) %>%
  rename(clip.onset = clip.onset.corrected) %>%
...
clip.summary.is$stthr.tri <- factor(
  clip.summary.is$stthr.tri,
  levels = c("midday", "morning", "afternoon"))
clip.summary.is$stthr.tri.a <- factor(
  clip.summary.is$stthr.tri,
  levels = c("afternoon", "midday", "morning"))
clip.summary.is$stthr.tri.o <- factor(
  clip.summary.is$stthr.tri,
  levels = c("morning", "afternoon", "midday"))

# ... and then with averages across clips at the recording level
rec.summary.is <- clip.summary.is %>%
  group_by(aclew_id, tier.name,
           mated.std, mated.bin, motyr.std, nsb.std, hsz.std) %>%
  summarize(
    mean.mph = mean(min.p.hr),
...
# corr btwn OTR and CHI at clip and recording levels
clip.CHI.OTR.rates.is <- clip.summary.is %>%
  select(-tot.spch.min, -nz.spch) %>%
  pivot_wider(names_from = "tier.name", values_from = "min.p.hr")
    
rec.CHI.OTR.rates.is <- rec.summary.is %>%
  select(-median.mph, -min.mph, -max.mph, -sd.mph, -total.clips) %>%
  pivot_wider(names_from = "tier.name", values_from = "mean.mph")

OTR.mean.is <- mean(rec.CHI.OTR.rates.is$OTR)
OTR.sd.is <- sd(rec.CHI.OTR.rates.is$OTR)

# rec.CHI.OTR.rates.is <- rec.CHI.OTR.rates.is %>%
#   mutate(
#     Tertile = factor(case_when(
#       OTR <= OTR.mean - OTR.sd ~ "low TCDS",
#       OTR >= OTR.mean + OTR.sd ~ "high TCDS",
...
```{r summarize_isVSnonis_OTR, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
mini.rec.summary <- rec.summary %>%
  ungroup() %>%
  select(aclew_id, tier.name, mean.mph) %>%
  filter(tier.name == "OTR") %>%
  rename("mean.tcds.mph" = "mean.mph")
mini.rec.summary.is <- rec.summary.is %>%
  ungroup() %>%
  select(aclew_id, tier.name, mean.mph) %>%
  filter(tier.name == "OTR") %>%
  rename("mean.is.tcds.mph" = "mean.mph")
# leave NAs in because prop isn't sensible for cases with zero TCDS
mini.rec.summary.all <- mini.rec.summary %>%
  full_join(mini.rec.summary.is) %>%
  mutate(prop.tcds.wi.is = mean.is.tcds.mph / mean.tcds.mph) %>%
  filter(!is.na(prop.tcds.wi.is))
```
...
On average, across all `r length(unique(rec.summary$aclew_id))` recordings from all `length(unique(rec.summary$aclew_id)) - 1` children, the baseline rate of speech directly addressed to the target child was `round(mean(rec.CHI.OTR.rates$OTR),2)` minutes per hour (median = `round(median(rec.CHI.OTR.rates$OTR),2)`, sd = `round(sd(rec.CHI.OTR.rates$OTR),2)`, range = `round(min(rec.CHI.OTR.rates$OTR),2)`--`round(max(rec.CHI.OTR.rates$OTR),2)`). This is somewhat lower than the average of 3.6 min/hr reported in the smaller sample from this community reported in Casillas et al. (2020). If we only count TCDS that occurs within turn-taking sequences (i.e., a prompt to a child's response, a response to a child's prompt, or an utterance within part of a longer turn-taking sequence), the baseline rate is slightly lower at `round(mean(rec.CHI.OTR.rates.is$OTR),2)` minutes per hour (median = `round(median(rec.CHI.OTR.rates.is$OTR),2)`, sd = `round(sd(rec.CHI.OTR.rates.is$OTR),2)`, range = `round(min(rec.CHI.OTR.rates.is$OTR),2)`--`round(max(rec.CHI.OTR.rates.is$OTR),2)`). As is apparent, most of the TCDS comes within turn-taking sequences. Across recordings, the mean proportion of TCDS within turn-taking sequences was `r round(mean(mini.rec.summary.all$prop.tcds.wi.is),2)` (median = `r round(median(mini.rec.summary.all$prop.tcds.wi.is),2)`, sd = `r round(sd(mini.rec.summary.all$prop.tcds.wi.is),2)`, range = `r round(min(mini.rec.summary.all$prop.tcds.wi.is),2)`--`r round(max(mini.rec.summary.all$prop.tcds.wi.is),2)`).
