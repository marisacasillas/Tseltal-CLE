...
On average, across all `r length(unique(rec.summary.all$aclew_id))` recordings from all `length(unique(rec.summary.all$aclew_id)) - 1` children, the baseline rate of speech directly addressed to the target child (i.e., TCDS) was `round(grand.means.OTR.all$mean.tot.mph,2)` minutes per hour (median = `round(grand.means.OTR.all$median.tot.mph,2)`, sd = `round(grand.means.OTR.all$sd.tot.mph,2)`, range = `round(grand.means.OTR.all$min.tot.mph,2)`--`round(grand.means.OTR.all$max.tot.mph,2)`). This is somewhat lower than the average of 3.6 min/hr reported in the smaller sample from this community reported in Casillas et al. (2020). If we only count TCDS that occurs within turn-taking sequences (i.e., a prompt to a child's response, a response to a child's prompt, or an utterance within part of a longer turn-taking sequence), the baseline rate is lower, at `round(grand.means.OTR.is$mean.tot.mph,2)` minutes per hour (median = `round(grand.means.OTR.is$median.tot.mph,2)`, sd = `round(grand.means.OTR.is$sd.tot.mph,2)`, range = `round(grand.means.OTR.is$min.tot.mph,2)`--`round(grand.means.OTR.is$max.tot.mph,2)`).
