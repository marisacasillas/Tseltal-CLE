...
# I give up on trying to do this inline with rename_all/rename_at womp womp
CHI.is.cols <- rec.summary.is %>%
  ungroup() %>%
  filter(tier.name == "CHI") %>%
  select(c(recording, tot.spchmin, mean.tot.mph))
colnames(CHI.is.cols) <- paste(names(CHI.is.cols), ".is", sep = "")

rec.means.prop.IS.CHI <- rec.summary.all %>%
  filter(tier.name == "CHI") %>%
  full_join(CHI.is.cols, by = c("recording" = "recording.is")) %>%
  ungroup() %>%
  mutate(prop.is.tot.spchmin = tot.spchmin.is/tot.spchmin) %>%
  pull(prop.is.tot.spchmin)
...
## TCDS ####
OTR.only <- subset(clip.summary.all, tier.name == "OTR")
tcds.random.distribution <- ggplot(OTR.only,
...
#sd(round(OTR.only$tot.min.p.hr,0))^2
#mean(round(OTR.only$tot.min.p.hr,0))
...
tcds.plot.age <- ggplot(
  tcds.mdl.pred.age, aes(x = age.mo, y = predicted)) +
  geom_ribbon(aes(ymax = conf.high, ymin = conf.low), alpha = 0.2) +
  geom_line() +
  labs(x = "Edad del niño (meses)", y = "Valores predichos (min/hr)") +
  geom_boxplot(data = OTR.only, aes(x = age, y = tot.min.p.hr,
                   color = factor(aclew_id), fill = factor(aclew_id)),
               outlier.fill = NA, alpha = 0.1,
               position = position_dodge()) +
  geom_jitter(data = OTR.only, aes(x = age, y = tot.min.p.hr,
                   color = factor(aclew_id)), alpha = 0.2) +
...
# Household effect
hsz.sd <- sd(rec.info$household_size)
hsz.mean <- mean(rec.info$household_size)

tcds.mdl.pred.hsz <- tibble(
  ggpredict(tds.rand.zinb, "hsz.std")) %>%
  mutate(household_size = (x * hsz.sd) + hsz.mean)

tcds.plot.hsz <- ggplot(tcds.mdl.pred.hsz, aes(x = household_size, y = predicted)) +
  geom_ribbon(aes(ymax = conf.high, ymin = conf.low), alpha = 0.2) +
  geom_line() +
  labs(x = "# Personas viviendo en el hogar",
       y = "Valores predichos (min/hr)") +
  geom_boxplot(data = OTR.only, aes(x = household_size, y = tot.min.p.hr,
                   color = factor(aclew_id), fill = factor(aclew_id)),
               outlier.fill = NA, alpha = 0.1,
               position = position_dodge(width = 2)) +
  geom_jitter(data = OTR.only, aes(x = household_size, y = tot.min.p.hr,
                   color = factor(aclew_id)), alpha = 0.2) +
  ...
  guides(color = "none", fill = "none") +
  theme_apa()
ggsave("TCDS_over_householdsize_with_estimates.png",
       plot = tcds.plot.hsz,
       width = 6, height = 3.5, dpi = 300)
...
<!-- PART 2
x how much speech do children produce?
    x does it change with age?
    x how much of it occurs within tt/is?
    x does volubility correlate with their directed input?
        x does it change with age?
        x what input counts?

-->

On average, across all recordings, the baseline rate of target child vocalization was `r round(grand.means.CHI.all$mean.tot.mph,2)` minutes per hour (median = `r round(grand.means.CHI.all$median.tot.mph,2)`, SD = `r round(grand.means.CHI.all$sd.tot.mph,2)`, range = `r round(grand.means.CHI.all$min.tot.mph,2)`--`r round(grand.means.CHI.all$max.tot.mph,2)`). In other words, children vocalize much more often than they encounter directed speech. Focusing now solely on vocalizations that occur within turn-taking sequences, the baseline rate is much lower, nearly matching the rate of TCDS children hear, at `r round(grand.means.CHI.is$mean.tot.mph,2)` minutes per hour (median = `r round(grand.means.CHI.is$median.tot.mph,2)`, SD = `r round(grand.means.CHI.is$sd.tot.mph,2)`, range = `r round(grand.means.CHI.is$min.tot.mph,2)`--`r round(grand.means.CHI.is$max.tot.mph,2)`). Unlike TCDS, which primarily occurs in turn-taking sequences, child vocalizations primarily occur outside of these sequences. Across recordings, the mean proportion of child vocalization occurring within turn-taking sequences was `r round(mean(rec.means.prop.IS.CHI),2)` (median = `r round(median(rec.means.prop.IS.CHI),2)`, sd = `r round(sd(rec.means.prop.IS.CHI),2)`, range =  `r round(min(rec.means.prop.IS.CHI),2)`--`r round(max(rec.means.prop.IS.CHI),2)`).

```{r cvoc_regression, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
## CVOC ####
OTR.minimal <- OTR.only %>%
  select(recording, segment.num, tot.min.p.hr,
         min.p.hr_mujer, `min.p.hr_niños`, min.p.hr_hombre) %>%
  rename(otr.tot.min.p.hr = tot.min.p.hr,
         otr.min.p.hr_mujer = min.p.hr_mujer,
         `otr.min.p.hr_niños` = `min.p.hr_niños`,
         otr.min.p.hr_hombre = min.p.hr_hombre)
CHI.only <- subset(clip.summary.all, tier.name == "CHI") %>%
  full_join(OTR.minimal)

cvoc.random.distribution <- ggplot(CHI.only,
                            aes(round(tot.min.p.hr, 0))) +
  geom_histogram(binwidth = 2) +
  ylab("# of clips") +
  xlab ("Child vocalization (min/hr)") +
  basic.theme
ggsave("CVOC_random_distribution.png",
       plot = cvoc.random.distribution,
       width = 8, height = 6, dpi = 300)
#sd(round(CHI.only$tot.min.p.hr,0))^2
#mean(round(CHI.only$tot.min.p.hr,0))
# mean is much smaller than variance

cvc.rand.zinb <- glmmTMB(round(tot.min.p.hr, 0) ~
                           tchiyr.std +
                           otr.tot.min.p.hr +
                           mated.bin +
                           tchiyr.std:otr.tot.min.p.hr +
                           (1|aclew_id),
                         data = CHI.only,
                         ziformula = ~tchiyr.std,
                         family="nbinom1")
# summary(cvc.rand.zinb)
  #    AIC      BIC   logLik deviance df.resid 
  # 2291.8   2328.1  -1136.9   2273.8      405 

# is TCDS rate just a proxy for TCDS from women?
# similar effect!
cvc.rand.zinb.alt <- glmmTMB(round(tot.min.p.hr, 0) ~
                           tchiyr.std +
                           otr.min.p.hr_mujer +
                           mated.bin + # mated.std effect * none vs. postsec
                           tchiyr.std:otr.min.p.hr_mujer +
                           (1|aclew_id),
                         data = CHI.only,
                         ziformula = ~tchiyr.std,
                         family="nbinom1")
# summary(cvc.rand.zinb.alt)
  #    AIC      BIC   logLik deviance df.resid 
  # 2256.7   2292.9  -1119.4   2238.7      405
# This is actually the better model
# anova(cvc.rand.zinb, cvc.rand.zinb.alt)

# how about for for TCDS from other children?
# similar effect!
cvc.rand.zinb.alt2 <- glmmTMB(round(tot.min.p.hr, 0) ~
                           tchiyr.std +
                           `otr.min.p.hr_niños` +
                           mated.bin +
                           tchiyr.std:`otr.min.p.hr_niños` +
                           (1|aclew_id),
                         data = CHI.only,
                         ziformula = ~tchiyr.std,
                         family="nbinom1")
# summary(cvc.rand.zinb.alt2)
  #    AIC      BIC   logLik deviance df.resid 
  # 2329.1   2365.3  -1155.5   2311.1      405 

# how about for for TCDS from men?
# no!
cvc.rand.zinb.alt3 <- glmmTMB(round(tot.min.p.hr, 0) ~
                           tchiyr.std +
                           otr.min.p.hr_hombre +
                           mated.bin +
                           tchiyr.std:otr.min.p.hr_hombre +
                           (1|aclew_id),
                         data = CHI.only,
                         ziformula = ~tchiyr.std,
                         family="nbinom1")
# summary(cvc.rand.zinb.alt3)

cvc.mdl.tbl <- broom.mixed::tidy(cvc.rand.zinb)
cvc.mdl.tbl.alt <- broom.mixed::tidy(cvc.rand.zinb.alt)
```

As with TCDS, many clips contained zero child vocalizations and, among the remainder of the clips, there was a skewed, non-negative distribution, so we again used a zero-inflated negative-binomial mixed-effects regression to model child vocalization rate. 


The conditional model (of non-zero integer minutes per hour of child vocalization) included a random effect of recording and predictors that have previously been found or theorized to influence input rate, including: target child age (numeric; standardized), time of day (factorial; morning vs. midday/afternoon), household size (numeric; standardized), maternal education level (factorial; <6 years vs. 6+ years of schooling), and an interaction between age and time of day. The zero-inflation component of the regression included a predictor of child age (numeric; standardized) on the prediction that younger children are overall more likely to hear zero TCDS in a given clip. The conditional regression revealed only two significant effects on TCDS rate: children hear more TCDS as they get older ($b$ = `r printnum(tds.mdl.tbl$estimate[which(tds.mdl.tbl$term == "tchiyr.std")[1]], digits = 3)`, $SE$ = `r printnum(tds.mdl.tbl$std.error[which(tds.mdl.tbl$term == "tchiyr.std")[1]], digits = 3)`, $t$ = `r printnum(tds.mdl.tbl$statistic[which(tds.mdl.tbl$term == "tchiyr.std")[1]], digits = 3)`, $p$ = `r printnum(tds.mdl.tbl$p.value[which(tds.mdl.tbl$term == "tchiyr.std")[1]],digits = 3)`), and children from larger households hear more TCDS ($b$ = `r printnum(tds.mdl.tbl$estimate[which(tds.mdl.tbl$term == "hsz.std")[1]], digits = 3)`, $SE$ = `r printnum(tds.mdl.tbl$std.error[which(tds.mdl.tbl$term == "hsz.std")[1]], digits = 3)`, $t$ = `r printnum(tds.mdl.tbl$statistic[which(tds.mdl.tbl$term == "hsz.std")[1]], digits = 3)`, $p$ = `r printnum(tds.mdl.tbl$p.value[which(tds.mdl.tbl$term == "hsz.std")[1]],digits = 3)`). We did not see a significant interaction of child age and time of day, as found in past work (Casillas et al., 2020). The zero-inflation component confirmed that clips from older children are significantly less likely to contain zero TCDS ($b$ = `r printnum(tds.mdl.tbl$estimate[which(tds.mdl.tbl$term == "tchiyr.std")[2]], digits = 3)`, $SE$ = `r printnum(tds.mdl.tbl$std.error[which(tds.mdl.tbl$term == "tchiyr.std")[2]], digits = 3)`, $t$ = `r printnum(tds.mdl.tbl$statistic[which(tds.mdl.tbl$term == "tchiyr.std")[2]], digits = 3)`, $p$ = `r printnum(tds.mdl.tbl$p.value[which(tds.mdl.tbl$term == "tchiyr.std")[2]],digits = 3)`).
